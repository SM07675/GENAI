"""Context manager — rolling context window and memory integration.

Maintains per-session conversation context with:
- Rolling turn history with auto-summarisation
- Short-term entity tracking (locations, apps, queries)
- Long-term memory integration via companion_db
- Interrupted turn preservation
"""
...
log = structlog.get_logger("genie.engine.context")
...
class ContextManager:
    ...
    The conversation engine creates one ContextManager per session and
    passes it to the orchestrator for every LLM call.
    """
    ...
    def resolve_references(self, text: str) -> str:
        ...
        Examples:
            ...
            "play another" → "play more jazz" (if last query was jazz)
        """
        text_lower = text.lower()
        resolved = text
    ...
    def get_last_assistant_text(self) -> Optional[str]:
        """Get the last assistant response for 'repeat' commands."""
        return self.last_assistant_text

    def clear(self) -> None:
        """Clear all context (used by 'clear history' command)."""
        self._turns.clear()
...
class ContextStore:
    """Manages ContextManager instances across sessions."""
    ...
    def get(self, session_id: str) -> ContextManager:
        if session_id not in self._sessions:
            if len(self._sessions) >= self.MAX_SESSIONS:
                self._evict_oldest()
            self._sessions[session_id] = ContextManager(session_id)
