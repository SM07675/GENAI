"""Intent router — fast local classification for deterministic commands.

Single wake-phrase stripping (no double-strip). Routes deterministic
intents locally and sends everything else to the LLM.
"""
...
log = structlog.get_logger("genie.engine.brain.intent")
...
# Wake phrases to strip (applied ONCE, only here)
_WAKE_PHRASES = [
...
class IntentRouter:
    ...
    This is the SINGLE place where wake phrases are stripped from user text.
    The orchestrator no longer strips them independently.
    """
    ...
    def strip_wake_phrase(self, text: str) -> str:
        ...
        This is the SINGLE authoritative place for wake phrase removal.
        """
        t = text.strip()
        lower = t.lower()
